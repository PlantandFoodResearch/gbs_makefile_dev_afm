...
INPUTFOLDER="/dataset/semihybrid_ryegrass_GBS/active/"$USER"/02_TagCounts/02_MergedTagCounts" # Where the input sequence files reside
OUTPUTFOLDER="/dataset/semihybrid_ryegrass_GBS/active/"$USER"/02_TagCounts/03_TagCountToFastq" # Where output of script should go
INPUTFILE="All_Ryegrass_PSTI_Testing" # Base name of Input tag counts files. 
OUTPUTFILE="All_Ryegrass_PSTI_Testing" # 
...
run_pipeline.pl  "$MINRRAM" "$MAXRRAM" -configFile "$OUTPUTFOLDER"/"$PROJECT"_TagCountToFastq.xml 2>&1 | tee -a "$OUTPUTFOLDER"/"$PROJECT"_TagCountToFastq_"$DATE".log
date | tee -a "$OUTPUTFOLDER"/"$PROJECT"_TagCountToFastq_"$DATE".log
...
echo "md5sum of fastq File" | tee -a "$OUTPUTFOLDER"/"$PROJECT"_TagCountToFastq_"$DATE".log
