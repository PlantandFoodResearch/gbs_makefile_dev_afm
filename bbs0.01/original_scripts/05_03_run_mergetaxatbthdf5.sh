...
INPUTFOLDER="/dataset/semihybrid_ryegrass_GBS/active/"$USER"/05_TBT/02_MergedTBT" # Where the input sequence files reside
OUTPUTFOLDER="/dataset/semihybrid_ryegrass_GBS/active/"$USER"/05_TBT/03_MergedTaxaTBT" # Where output of script should go
INPUTFILE="mergeTBTHDF5" # Base name of Input HDF5 files. (outputfile from run_mergetbthdf5tbthdf5.sh)
OUTPUTFILE="mergeTBTHDF5_mergedtaxa" # Base name of Output HDF5 file.
...
#######
  ...
  run_pipeline.pl  "$MINRRAM" "$MAXRRAM" -configFile "$OUTPUTFOLDER"/"$PROJECT"_MERGETaxaTBTHDF5.xml 2>&1 | tee -a "$OUTPUTFOLDER"/"$PROJECT"_MERGETAXATBTHDF5_"$DATE".log
date | tee -a "$OUTPUTFOLDER"/"$PROJECT"_MERGETAXATBTHDF5_"$DATE".log
